...
# get config obj with absolute path
def get_config_obj():
...
def Channel_generation(N_devices, N_IRS, RIS_loc, FC_loc, FC_RIS_dis, L_0, alpha, len_sample, K=8):
    ...
    Channel_sequence = (torch.normal(mean=0., std=torch.tensor(np.sqrt(2) / 2) * torch.ones(len_sample, (
            N_devices + 1) * N_IRS + N_devices)) * 1j + torch.normal(mean=0.,
                                                                     std=torch.tensor(np.sqrt(2) / 2) * torch.ones(
                                                                         len_sample, (
                                                                                             N_devices + 1) * N_IRS + N_devices))).cuda()  # N_devices+N_device*N_IRS+N_IRS
    # Channel_sequence[:,N_devices:] = Channel_sequence[:,N_devices:]*np.sqrt(1/(1+K))+np.sqrt(K/(1+K))*1
    for j in range(N_devices):
        Channel_sequence[:, j] = Channel_sequence[:, j] * db2pow(L_0) * torch.sqrt(
            (dis_matrix[j, 0]) ** (-alpha[0]))  # FC-User
        Channel_sequence[:, N_devices + j * N_IRS:N_devices + (j + 1) * N_IRS] = Channel_sequence[:,
                                                                                 N_devices + j * N_IRS:N_devices + (
                                                                                         j + 1) * N_IRS] * np.sqrt(
            1 / (1 + K)) + torch.tensor(np.sqrt(K / (1 + K)) * H_irs_user[j, :], dtype=torch.complex128, device=0)
        Channel_sequence[:, N_devices + j * N_IRS:N_devices + (j + 1) * N_IRS] = Channel_sequence[:,
                                                                                 N_devices + j * N_IRS:N_devices + (
                                                                                         j + 1) * N_IRS] * db2pow(
            L_0) * torch.sqrt((dis_matrix[j, 1]) ** (-alpha[1]))  # FC-IRS
    for jj in range(N_IRS):
        if -N_IRS + jj + 1 == 0:
            Channel_sequence[:, -N_IRS + jj:] = Channel_sequence[:, -N_IRS + jj:] * np.sqrt(1 / (1 + K)) + torch.tensor(
                np.sqrt(K / (1 + K)) * H_fc_irs[-1], dtype=torch.complex128, device=0)
            Channel_sequence[:, -N_IRS + jj:] = Channel_sequence[:, -N_IRS + jj:] * db2pow(L_0) * np.sqrt(
                (FC_RIS_dis) ** (-alpha[2]))
        else:
            Channel_sequence[:, -N_IRS + jj:] = Channel_sequence[:, -N_IRS + jj:] * np.sqrt(1 / (1 + K)) + torch.tensor(
                np.sqrt(K / (1 + K)) * H_fc_irs[-N_IRS + jj:], dtype=torch.complex128, device=0)
            Channel_sequence[:, -N_IRS + jj:-N_IRS + jj + 1] = Channel_sequence[:,
                                                               -N_IRS + jj:-N_IRS + jj + 1] * db2pow(L_0) * np.sqrt(
                (FC_RIS_dis) ** (-alpha[2]))
    ...
    for jjj in range(N_devices):
        cascaded_channel[:, jjj, :] = Channel_sequence[:,
                                      N_devices + jjj * N_IRS:N_devices + (jjj + 1) * N_IRS] * Channel_sequence[:,
                                                                                               -N_IRS:]
...
def Received_pilots_generation(phase_shifts, pilots, len_sample, N_devices, N_IRS, pilots_length, Channel_sequence,
                               noise=-85):
    len_frame = N_devices
    num_frame = pilots_length // len_frame
    # Recived Pilots
    ...
    for frame in range(num_frame):
        Received_pilots[:, frame * N_devices:(frame + 1) * N_devices] = combined_channel[:, :, frame].squeeze(
            -1) @ pilots[frame * N_devices:(frame + 1) * N_devices, :] + torch.tensor(np.sqrt(2) / 2) * torch.normal(
            mean=0, std=np.sqrt(dbm2pw(noise)) * torch.ones(len_sample, N_devices)).cuda() + 1j * torch.tensor(
            np.sqrt(2) / 2) * torch.normal(mean=0,
                                           std=np.sqrt(dbm2pw(noise)) * torch.ones(len_sample, N_devices)).cuda()
    return combined_channel, phase_shifts, Received_pilots


def Data_generation(N_devices, N_IRS, RIS_loc, FC_loc, FC_RIS_dis, L_0, alpha, len_sample, pilots_length, K=8,
                    noise=110, transmit_p=1):
    ...
    combined_channel, phase_shifts, Received_pilots = Received_pilots_generation(phase_shifts, pilots, len_sample,
                                                                                 N_devices, N_IRS, pilots_length,
                                                                                 Channel_sequence, noise=noise)
