...
from data.utils.generate_channel import generate_channel, channel_complex2real, generate_channel_with_array_response
...
def test_channel_estimation_lmmse(params_system, len_pilot, noise_power_db, location_user, Rician_factor, num_sample,
    ...
    # ===channel estimation===
    channel_bs_user_est, channel_bs_irs_user_est, y_decode = channel_estimation_lmmse(params_system, y, pilots,
                                                                                      phase_shifts, stat_info)
...
def channel_estimation(params_system, len_pilot, noise_power_db, location_user, Rician_factor, num_sample,
                       pilot_power=15, max_iter=20, dir='./DC/plot_data', location_bs=np.array([-80, 0, 30]),
                       location_irs=np.array([10, 0, 10]), L_0=-30, alpha=[3.6, 2.5, 2.2], Estimated=True):
    (num_antenna_bs, num_elements_irs, N_devices) = params_system
    ...
    for ii in range(max_iter):
        ...
        channels, set_location_user = generate_channel(params_system, location_bs=location_bs,
                                                       location_irs=location_irs, num_samples=num_sample,
    ...
    scio.savemat(dir + '/Channel_estimated_LMMSE_data_%d_%d_%d_%d_equalSNR_0.mat' % (
    N_devices, len_pilot, pilot_power, num_elements_irs), {'Channel_estimated_data_LMMSE': Channel_data})
    scio.savemat(dir + '/Channel_real_LMMSE_data_%d_%d_%d_%d_equalSNR_0.mat' % (
    N_devices, len_pilot, pilot_power, num_elements_irs), {'Channel_real_data_LMMSE': Channel_data_real})
    scio.savemat(
        dir + '/Received_pilots_%d_%d_%d_%d_equalSNR_0.mat' % (N_devices, len_pilot, pilot_power, num_elements_irs),
        {'Received_pilots': Received_pilots})


def channel_generation(params_system, len_pilot, noise_power_db, location_user, Rician_factor, num_sample,
                       pilot_power=15, location_bs=np.array([-80, 0, 30]), location_irs=np.array([10, 0, 10]), L_0=-30,
                       alpha=[3.6, 2.5, 2.2]):
    (num_antenna_bs, num_elements_irs, N_devices) = params_system
    phase_shifts, pilots = generate_pilots_bl(len_pilot, num_elements_irs, N_devices)
