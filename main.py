...
def evaluate(GNN, test_loader, num_device):
    ...
    with torch.no_grad():
        for batch_combined_channel in test_loader:
            batch_combined_channel = batch_combined_channel.cuda()
            input = (batch_combined_channel[:, :, 0].view(batch_combined_channel.size(0), 1, num_device) +
                     batch_combined_channel[:, :, 1:].sum(-1).view(batch_combined_channel.size(0), 1, num_device))
            channel_compose = torch.cat((input.real, input.imag), 1).cuda()
            Net_output = GNN.forward(channel_compose).clone()
            ...
            c_sum = torch.sum(c, dim=1).unsqueeze(1).cuda()
            mean_class_1_8 = torch.tensor(mean_class).cuda().reshape((1, num_class * num_feature))
            mu_hat = (c_sum @ mean_class_1_8).reshape((batch_combined_channel.size(0), num_class, num_feature))

            sigma_hat = torch.zeros(batch_combined_channel.size(0), num_feature).cuda()
            temp = c_sum ** 2 @ torch.tensor(var_class).cuda().unsqueeze(0) ** 2
            ...
            discgain = torch.zeros((batch_combined_channel.size(0), int(num_class * (num_class - 1) / 2),
                                    num_feature)).cuda()
...
def main():
    ...
    for num_device in num_device_list:
        train_dataset = CustomDataset(config['data']['train_dir'] + f'/train_combined_channel_device{num_device}.npy')
        train_loader = DataLoader(train_dataset, batch_size=config['training']['batch_size'], shuffle=True)
        test_dataset = CustomDataset(config['data']['test_dir'] + f'/test_combined_channel_device{num_device}.npy')
