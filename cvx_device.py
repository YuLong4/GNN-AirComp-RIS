...
def main():
    ...
    def SCA_for_two_PCA(alpha_init, c_zf_init, f_vec_init, v_bar_init, num_antenna, power_tx, channel_gain_hd,
                        channel_gain_hr, channel_gain_R, num_class, num_ris, num_device, idx):
        ...
        while (diff > 1e-2 and count < 100):
            ...
            for idx_device in range(num_device):
                ...
                constrains += [
                    c_zf[idx_device] ** 2 + (2 * Y[idx_device] @ v_bar_init - 2 * (Z[idx_device].reshape(-1, 1))).T @ (
    ...
    def SCA_c_Rf(alpha_init, c_zf, v_bar, f_vec_init, num_antenna, power_tx, channel_gain_hd, channel_gain_hr,
                 channel_gain_R, num_class, num_ris, num_device, idx):
                ...
                for idx_class_b in range(idx_class_a):
                    for idx_m in range(2):
                        constrains += [np.sum(np.multiply(c_zf, np.sqrt(var_dist[:, 2 * idx + idx_m].reshape(-1, 1))) ** 2)
                            ...
                            idx_class_b, 2 * idx + idx_m]) ** 2) / (
                                           alpha_init[
                                               int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b, idx_m]) ** 2 * (
    ...
    for i in range(len(num_device_list)):
        ...
        for j in range(1, num_device):
            rng2 = default_rng()
            rng3 = default_rng()
            rayli_fading_real_hd = np.hstack(
                (rayli_fading_real_hd, rng2.normal(0, 1, (num_antenna, 1))))  # rayleigh fading ~ CN(0,1)
        ...
        for j in range(1, num_device):
            rng2 = default_rng()
            rng3 = default_rng()
            rayli_fading_real_hr = np.hstack(
                (rayli_fading_real_hr, rng2.normal(0, 1, (num_ris, 1))))  # rayleigh fading ~ CN(0,1)
