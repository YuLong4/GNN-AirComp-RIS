...
class User_block(nn.Module):
    ...
    def forward(self, x):
        ...
        # combination operation
        result = torch.cat((output_aggregate, self.irs_net(x[:, -2, :]), self.bs_net(x[:, -1, :]), x[:, self.order, :]),
                           1)
...
class Graph_net(nn.Module):
    def __init__(self, size_input, number_node, number_irs_elements, number_layer=2, size_hidden=[64, 256, 128]):
        ...
        self.output_layers_eta = nn.Linear(size_hidden[number_layer], 2).double().cuda()
    ...
    def forward(self, x):
        ...
        result = torch.zeros(batch, self.number_node + self.number_irs_elements + 2).cuda().double()
        ...
        result[:, -2:] = self.output_layers_eta(x[:, -1, :]).squeeze()
