...
def main():
    ...
    return_channel_gain_hd = channel_gain_hd
    return_channel_gain_hr = channel_gain_hr
    return_channel_gain_R = channel_gain_R
    ...
    for k in range(num_device):
        channel_gain_init = channel_gain_hd[:, k].reshape(-1, 1) + channel_gain_R @ np.diag(
            channel_gain_hr[:, k]) @ v_bar_init[0:num_ris, ]
    ...
    # two dimension's PCA
    ##############################################################
    def SCA_for_two_PCA(alpha_init, c_zf_init, f_vec_init, v_bar_init, num_antenna, power_tx, channel_gain_hd,
                        channel_gain_hr, channel_gain_R, num_class, num_ris, num_device):
        for idx_class_a in range(num_class):
            for idx_class_b in range(idx_class_a):
                for idx_m in range(2):
                    alpha_init[int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b, idx_m] = (
                            ((mean_class[idx_class_a, 2 * idx + idx_m] - mean_class[idx_class_b, 2 * idx + idx_m]) ** 2) / (
                            (np.sum(var_dist[:, 2 * idx + idx_m].reshape(1, num_device) @ (c_zf_init ** 2)) + var_comm_noise
                             * np.sum(f_vec_init ** 2)) / ((np.sum(c_zf_init)) ** 2) + var_class[2 * idx + idx_m]))
        ...
        while (diff > 5e-2 and count < 100):
            ...
            for idx_device in range(num_device):
                ...
                constrains += [c_zf[idx_device] ** 2 + (2 * Y[idx_device] @ v_bar_init - 2
                                                        * (Z[idx_device].reshape(-1, 1))).T @ (
                                       v_bar - v_bar_init.reshape(-1)) <= 0]

            for idx_class_a in range(num_class):
                for idx_class_b in range(idx_class_a):
                    for idx_m in range(2):
                        constrains += [
                            (cp.sum_squares(cp.multiply(c_zf, np.sqrt(var_dist[:, 2 * idx + idx_m].reshape(-1, 1))))
                             + var_comm_noise * np.sum(f_vec_init ** 2))
                            + (cp.sum(c_zf)) ** 2 * var_class[2 * idx + idx_m]
                            - np.sum(c_zf_init) ** 2 * ((mean_class[idx_class_a, 2 * idx + idx_m] - mean_class[
                                idx_class_b, 2 * idx + idx_m]) ** 2 / alpha_init[int(idx_class_a * (
                                    idx_class_a - 1) / 2) + idx_class_b, idx_m]) - 2 * (
                                    mean_class[idx_class_a, 2 * idx + idx_m] - mean_class[
                                idx_class_b, 2 * idx + idx_m]) ** 2 * np.sum(c_zf_init) / alpha_init[
                                int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b, idx_m] * cp.sum(
                                c_zf - c_zf_init) + (np.sum(c_zf_init) * (
                                    mean_class[idx_class_a, 2 * idx + idx_m] - mean_class[
                                idx_class_b, 2 * idx + idx_m]) / alpha_init[int(idx_class_a * (
                                    idx_class_a - 1) / 2) + idx_class_b, idx_m]) ** 2 * (
                                    alpha[idx_m][int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b] - alpha_init[
                                int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b, idx_m]) <= 0]
    ...
    #################### Second round SCA #######################
    def SCA_c_Rf(alpha_init, c_zf, v_bar, f_vec_init, num_antenna, power_tx, channel_gain_hd, channel_gain_hr,
                 channel_gain_R, num_class, num_ris, num_device):
        ...
        for idx_class_a in range(num_class):
            for idx_class_b in range(idx_class_a):
                for idx_m in range(2):
                    alpha_init[int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b, idx_m] = (
                            ((mean_class[idx_class_a, 2 * idx + idx_m] - mean_class[idx_class_b, 2 * idx + idx_m]) ** 2) / (
                            (np.sum(var_dist[:, 2 * idx + idx_m].reshape(1, num_device) @ (c_zf_init ** 2)) +
                             var_comm_noise * np.sum(f_vec_init ** 2)) /
                            ((np.sum(c_zf_init)) ** 2) + var_class[2 * idx + idx_m]))
        ...
        while (diff > 5e-2 and count < 100):
            for idx_device in range(num_device):
                constrains += [
                    2 * power_tx * channel_gain_device[idx_device] @ f_vec_init.reshape(-1) * f_vec_init.reshape(-1).T @
                    channel_gain_device[idx_device].T
                    + 4 * power_tx * channel_gain_device[idx_device] @ channel_gain_device[
                        idx_device].T * f_vec_init.reshape(-1) @ (f_vec - f_vec_init.reshape(-1)) - c_zf[
                        idx_device] ** 2 >= 0]
                ...
                for idx_class_b in range(idx_class_a):
                    for idx_m in range(2):
                        constrains += [np.sum(np.multiply(c_zf, np.sqrt(var_dist[:, 2 * idx + idx_m].reshape(-1, 1))) ** 2)
                                       ...
                                       - np.sum(c_zf) ** 2 * ((mean_class[idx_class_a, 2 * idx + idx_m] - mean_class[
                            idx_class_b, 2 * idx + idx_m]) ** 2) / (
                                           alpha_init[int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b, idx_m])
                                       + np.sum(c_zf) ** 2 * ((mean_class[idx_class_a, 2 * idx + idx_m] - mean_class[
                            idx_class_b, 2 * idx + idx_m]) ** 2) / (
                                           alpha_init[int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b, idx_m]) ** 2
                                       * (alpha[idx_m][int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b] -
                                          alpha_init[int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b, idx_m]) <= 0]
    ...
    def add_noise_to_normed_pca(data_test_pca_normed, c_zf_init, f_vec_init):
        ...
        data_test_pca_add_noise = (np.sum(c_zf_init) * data_test_pca_normed + c_zf_init.T @ var_dist + np.sum(
            f_vec_init * var_comm_noise)) / np.sum(c_zf_init)
    ...
    for i in range(len(power_list)):
        ...
        for idx in range(0, int(PCA_dim / 2)):
            ...
            while (diff > 1e-2 and count < 100):
                c_zf, v_bar, discri, alpha_init = SCA_for_two_PCA(alpha_init, c_zf_init, f_vec_init, v_bar_init,
                                                                  num_antenna, power_tx, channel_gain_hd, channel_gain_hr,
                                                                  channel_gain_R, num_class, num_ris, num_device)
                c_zf_init, f_vec_init, alpha_init, discri = SCA_c_Rf(alpha_init, c_zf, v_bar, f_vec_init, num_antenna,
                                                                     power_tx, channel_gain_hd, channel_gain_hr,
                                                                     channel_gain_R, num_class, num_ris, num_device)
                v_bar_init = v_bar
            ...
            data_test_pca_add_noise[:, idx * 2:idx * 2 + 2] = add_noise_to_normed_pca(
                data_test_pca_normed[:, idx * 2:idx * 2 + 2], c_zf_init, f_vec_init)
        ...
        for k in range(num_device):
            channel_gain_init = channel_gain_hd[:, k].reshape(-1, 1) + channel_gain_R @ np.diag(channel_gain_hr[:, k]) @ (
                v_bar_init[0:num_ris, ]).reshape(-1, 1)
            c_zf_k = 2 * power_tx * channel_gain_init.T @ f_vec_init * f_vec_init.T @ channel_gain_init
                ...
                for idx_class_b in range(idx_class_a):
                    for idx_m in range(2):
                        alpha_init[int(idx_class_a * (idx_class_a - 1) / 2) + idx_class_b, idx_m] = (
                                ((mean_class[idx_class_a, 2 * idx + idx_m] - mean_class[idx_class_b, 2 * idx + idx_m]) ** 2)
                                / ((np.sum(var_dist[:, 2 * idx + idx_m].reshape(1, num_device) @ (c_zf_init ** 2))
                                    + var_comm_noise * np.sum(f_vec_init ** 2)) / ((np.sum(c_zf_init)) ** 2) +
                                   var_class[2 * idx + idx_m]))

            disc_init += 2 * alpha_init.sum() / num_class / (num_class - 1)
            data_test_pca_add_noise_baseline[:, idx * 2:idx * 2 + 2] = add_noise_to_normed_pca(
                data_test_pca_normed[:, idx * 2:idx * 2 + 2], c_zf_init, f_vec_init)

        discriminant_gain_baseline_list[i] = disc_init
    return return_channel_gain_hd, return_channel_gain_hr, return_channel_gain_R


def generate_channel_gain_samples(num_sample):
    count = 1
    ...
    while count <= num_sample:
        print(f"count: {count}")
        try:
            channel_gain_hd, channel_gain_hr, channel_gain_R = main()
            ...
            print(e)
            print('try again')
    np.savez(f'./data/from_cvx/power/channels/channels_power_general.npz',
             array1=channel_gain_hd_sample, array2=channel_gain_hr_sample, array3=channel_gain_R_sample)


if __name__ == "__main__":
    # main()
    generate_channel_gain_samples(50)
